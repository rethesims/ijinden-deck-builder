...
デッキの共有は「マイデッキ」のデッキコード (サーバー発行) を使います。
それとは別に、次の形の URL を渡されたときも読み込めるようにしてあります。

    <URL>/#/deck/<コード>

コードそのものにレシピの中身が入っているため、サーバーを介さずに復元できます。
インポート欄にこの URL を貼るか、URL を直接開くと反映されます。
**当アプリはこの形式のリンクを発行しません。** 読めるだけです。
仕様は `src/deckCode.js` のコメントを参照してください。

**コードはカードの `orderTable` を鍵にしています。**
`src/cards.json` を更新するときに `orderTable` を振り直すと、
過去に発行されたコードがすべて別のデッキを指してしまいます。
元サイトの値をそのまま使ってください。

カードデータが元サイトとずれていないかは次のコマンドで確認できます。
カードデータを更新したときと、ときどき実行してください。

```sh
python3 scripts/check-upstream-cards.py
```

元サイトの現在のデータと突き合わせて、既存カードの `orderTable` のずれ
(=過去のコードが黙って別のデッキを指す状態)、カードの増減、符号化方式の変更を
検出します。差異があれば終了コード1を返します。

未知の形式のコードを渡された場合は「読めないリンク」としてエラーになるだけで、
誤ったデッキを表示することはありません。
